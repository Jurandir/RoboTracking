...
const lista         = require('./listeRelacao')
    ...
    for await ( itn of lst ) {
        ...
        await sqlQuery(`SELECT * FROM SIC.dbo.NOTAFISCAL WHERE DANFE='${danfe}'`).then(ret=>{
            ...
            if(id1 && !id2) {
                ...
                iupd++
                console.log(`retransmite - <${a++}>`,itn.caIdCargaPk,itn.danfe,id1,id2)     
            }

            if(!id1 && !id2) {
                let upd = reinsert(danfe,idCarga)
                iIns++
                console.log(`reinsert + <${a++}>`,itn.caIdCargaPk,itn.danfe,id1,id2)     
            }
...
async function retransmite(danfe,idCarga) {
    ...
    let sql = `UPDATE NOTAFISCAL
                SET DT_ATUAL = CURRENT_TIMESTAMP
                    ${ysql}
                    ,DANFE_API    = 'Re-Envio Habilitado. 02.'
                    ,DT_UPDATE    = NULL
                    ${xsql}
                WHERE 
                DANFE = '${danfe}'`
